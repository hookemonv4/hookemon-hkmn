...
- `PhaseThreeReleasePlan` validates materialized seed calldata against the target hook's immutable digest and configured custody before a wallet action, including `deadline <= block.timestamp + 900`. `buildLaunchPackage()` and `verifyLaunchPackage()` render and verify the checked-in `ADDRESS_DERIVATION_PENDING` graph draft or a separately written materialized package; the verifier rederives the materialized address manifest, decodes the seed transaction, and rejects an intent-digest mismatch. The draft includes `create-request.json`, a deterministic local template for the 21 recorded V4 envelope fields. Values that committed evidence does not establish remain explicit `null`; the template is not a POST body. Verification may use `--allow-unverified` for local review; it does not change readiness.
...
- `scripts/programmable/preflight.mjs` loads the V4 request template from `HEAD`, checks the live capability deployment/profile binding against the pinned evidence, and uses only the advertised read-only preflight route. It requires `--repository-url`, `--source-commit`, and `--source-tree`; source flags prove the public repository, commit, and tree but do not format provider source-descriptor fields without the retained nested provider contract. An EVM account transaction count is not a V4 provider nonce. `--dry-run` prints a body only after all required fields are resolved, while `--status <requestId>` uses the advertised status route.
...
- Preflight sends only the 21 top-level fields allowed by `programmable.custom-launch-create-request.v4`. Its local contract check rejects missing fields, extra fields, unexpected schema constants, an unsupported funding mode, or a verification-bundle schema mismatch. Template nulls are permitted only for the committed draft; after live bindings, every unresolved path rejects before the POST. It copies `chainDeployment`, `chainDeploymentDescriptorDigest`, and `profile` from live capabilities only when they match the pinned evidence; it requires an independently established nonzero lowercase-bytes32 provider nonce and a permit window no longer than 900 seconds. It cannot create, submit, sign, or broadcast a launch.
...
- An accepted response writes evidence and prints a read-only handoff. A provider rejection also writes evidence, reports numbered differences, and exits nonzero. Status polling is read-only and does not alter evidence or provider state.
- A numbered format probe that reaches a deeper provider validation boundary records that field shape in `provider-documents.json`. A repeated source-manifest digest rejection preserves explicit nulls in the materialized request until the provider supplies its digest preimage.
...
node scripts/programmable/preflight.mjs --repository-url https://github.com/hookemonv4/hookemon-hkmn --source-commit "$(git rev-parse HEAD)" --source-tree "$(git rev-parse HEAD^{tree})"
node scripts/programmable/preflight.mjs --repository-url https://github.com/hookemonv4/hookemon-hkmn --source-commit "$(git rev-parse HEAD)" --source-tree "$(git rev-parse HEAD^{tree})" --dry-run
...
- OPEN FACT: `create-request.json` contains the recorded envelope and reproducible build evidence, but committed records do not establish the provider deployment descriptor, graph salts and call data, source descriptor, target runtime materialization, launch intent, or agent attestation. Resolve: retain the canonical provider capability copy and complete nested V4 schemas, then record the provider graph preimage and source commitment inputs. Verified alternative: retain the explicit-null template and stop before a provider POST.
- OPEN FACT: probes `008`, `009`, and `010` each returned `sourceBundleManifest digest does not match sourceDescriptor`. The accepted descriptor and manifest field shapes are recorded, but their binding recipe is not. Resolve it with the provider's V4 digest preimage or a provider-generated descriptor and repeat a read-only probe. The verified alternative is to retain null source commitment values and never treat a probe digest as launch evidence.
- OPEN FACT: probe `001` confirmed only that the V4 nonce is a nonzero lowercase bytes32. It did not establish a derivation from an EVM account transaction count. Resolve it with the provider's nonce rule or a provider-generated nonce and validate it in a read-only probe. The verified alternative is to retain a null nonce and reject a padded account count.
