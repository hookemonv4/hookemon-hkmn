...
The selected HKMN icon is `release/phase3/metadata/hkmn-logo.png`, a lossless PNG encoding of the owner-supplied JPEG pixels. Ancillary PNG metadata is excluded for the official V4 decoder. `feasibility/native-preflight-materialization/metadata-input.json` binds its immutable GitHub commit URL; release launch inputs retain its dimensions and content hash. The previous `comic/coin.png` is a website asset, not the selected launch icon. Icon validation does not establish request admission or DEX Screener publication.
