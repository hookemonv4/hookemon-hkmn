...
Synthetic proof tests may construct bindings only with the exact test-profile capability.
...
Native Solana return signing accepts the release-authenticated `deposit_token` grammar from Relay source `458a64c`: discriminator `0b9c60da27a3b413`, 48 bytes, little-endian amount at byte 8 and order ID at byte 16. The policy independently derives depository/vault PDAs and sender/vault USDC associated accounts, checks all ten account privileges, and binds the order's source and destination identities. It permits only one deposit plus bounded compute-budget limit/price instructions. Generic transaction decoding retains its unknown instruction classification; the exact original message bytes remain the signing policy's authority.

Full public keys from the frozen plan compile into a packet-bounded legacy message. ALT addresses remain metadata and never supply missing accounts; versioned messages are not admitted by this path. A finalized loader/ProgramData observation verifies the release-selected source executable immediately before signing and broadcasting. An upgrade, changed ELF, stale observation or cloned binding refuses that action. Main and supplementary returns use the same policy. Focused verification: `node --test packages/adapters/test/native/native-return-policy.test.mjs`. Runtime evidence and instruction accounts are documented in `docs/evidence/native-relay-runtime-build-20260908/` and `docs/evidence/native-relay-source-instruction-20260908/`.

The signing boundary consumes `hookemon.relay-intent.v2` from `prepareExecution`: its destination asset is the internal `native` literal on chain 4663 with 18 decimals. The provider's zero-address wire representation is not an internal intent alias.
