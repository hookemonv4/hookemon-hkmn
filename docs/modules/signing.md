...
- EVM decoding supports legacy and EIP-1559 envelopes. It rejects access lists, authorization lists, blob fields, other unmodeled typed-envelope fields, and unsafe JavaScript integer inputs. ERC-20 decoding requires explicit token metadata. Solana compute-unit prices use `native-microlamports-per-compute-unit` with 15 decimals. Solana v0 address lookup tables come from `lookupTableResolver` or `lookupTableRpc.getAddressLookupTable`; reads use finalized commitment and reject unresolved, mismatched, or inactive tables. `blockhashContextResolver` returns either the exact blockhash/`lastValidBlockHeight` pair or a strict `rpc-blockhash-validity` observation with the exact hash, `valid: true`, and canonical `observedSlot`. Purchase can bind the second variant without inventing an expiry height; its policy pins the original hash and minimum observation slot. Fresh decoding and signed-message recovery reject invalid, malformed, mismatched or regressing observations. Approval hashes exclude only the varying observation slot for this variant; the purchase RPC-validity policy digest includes the complete immutable rule sidecar and its minimum observation slot, so a reconstructed policy with a lower bound cannot recover an existing approval. The existing height variant and signature verification remain unchanged.
...
- Configure `packages/adapters/bin/hookemon-keychain-signer.mjs` only with the corresponding public account bindings. Its Solana signing and production transport are implemented; live use requires the frozen mutation authority and independently approved release anchors. Probe creates an internal signature and needs its own authorization. It accepts no secret configuration.
...
Purchase production bindings require `config.solana.originalBlockhashContextResolver`. Only the original message hash is extracted before independent RPC validation; amount, recipient, signer and memo still come from trusted binding and durable cycle facts. The old fixture-only height path remains available when no original resolver is configured. Provider message bytes are never rewritten.

For the approved Collector Core profile, `wrapTransactionPolicySignerClient` receives the independently configured `solanaOperatorAddress`. It verifies the provider signature in slot zero before Operations signs slot one and preserves that signature through broadcast revalidation and recovery. Legacy callers retain Operations in slot zero.

Production buyback refuses a missing or null binding registry before provider mutation. Reconciliation uses completed open and epic decision records to recover memos after a stage-level ambiguous send. At its durable send deadline, an unresolved outcome becomes attributed held custody only after current finalized ownership is verified and no known finalized outgoing transfer contradicts it; missing transports or unverified custody hold the cycle without adding a held asset; no provider regeneration or second sale occurs. Hold evidence identifies the failed verification step without copying transport error text. Cancellation and programming errors, including wrapped causes, propagate without a custody mutation. A successful Core signed-message recovery permits a later validity observation slot while retaining the original blockhash and policy minimum slot.

At an unresolved buyback deadline, transport failures stop the cycle without creating a held asset or available proceeds. Explicit cancellation propagates. A confirmed outgoing card transfer is recorded separately from incomplete ownership or settlement verification.
