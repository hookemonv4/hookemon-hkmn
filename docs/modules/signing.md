...
Production buyback refuses a missing or null binding registry before provider mutation. Reconciliation uses completed open and epic decision records to recover memos after a stage-level ambiguous send. A per-card unresolved outcome becomes attributed held custody at its durable send deadline; no provider regeneration or second sale occurs. A successful Core signed-message recovery permits a later validity observation slot while retaining the original blockhash and policy minimum slot.
