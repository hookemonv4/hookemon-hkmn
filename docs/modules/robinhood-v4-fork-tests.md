...
The active Hookemon archive fixtures settle native ETH through zero Currency; retained USDG-named local constants are historical labels for synthetic test integers, not dollar funding or a production seed selection. Source tests emit each actual token, hook, custody and graph hash before enforcing the exact pinned expectations. A changed native runtime must be measured in the authorized archive execution and reviewed before its expected hash changes. The archive test file and both workflow invocations bind the same file digest; a protected control-pin approval remains separate from updating that content address.
