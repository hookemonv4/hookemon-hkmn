...
- `.github/workflows/deploy-web.yml` deploys `apps/web` to the protected `production` GitHub Environment. It never triggers directly (no `push`, `pull_request`, or `workflow_dispatch`): it only fires on a completed `workflow_run` of `Hookemon CI`, and its job additionally asserts in shell that the triggering run's conclusion was `success`, its event was `push`, and its `head_branch` was `main`. It checks out the exact `github.event.workflow_run.head_sha` the Hookemon CI run validated, uploads that build as a Cloudflare Worker version, and promotes the uploaded version id (never a fresh build) to 100%.
