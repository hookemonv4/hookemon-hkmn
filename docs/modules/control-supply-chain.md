...
The control supply chain content-addresses selected Node and Gitleaks executables, remote actions, all eight CI workflows, scanner policy, the commit-identity checker, the complete fork-pin verifier import closure, the complete control-verifier import closure, the archive proof, and the vendored base pack. Verification fails closed on a mismatch.
...
- `.github/workflows/web-ci.yml` ("Hookemon CI") builds and tests `apps/web` on a pull request touching it and on a push to `main`; least-privilege `contents: read`, no secret access, never deploys.
...
- `web-ci` is intended as a required pull-request status once the owner registers it on `main`; it is not yet a branch-protection requirement. `deploy-web` is never a required or blocking status by design: it deploys, it does not gate merges.
