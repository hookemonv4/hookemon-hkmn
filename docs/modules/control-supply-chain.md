...
The control supply chain content-addresses selected Node and Gitleaks executables, remote actions, five CI workflows, scanner policy, the push-range resolver, the commit-identity checker, the complete fork-pin verifier import closure, the complete control-verifier import closure, the archive proof, and the vendored base pack. Verification fails closed on a mismatch.
...
- `node scripts/ci/push-range.mjs resolve <before-sha> <head-sha> <before|merge-base> <output-file> [summary-file]` resolves a push range and records an initial-push decision when applicable.
- `node scripts/ci/push-range.mjs append-only <empty-tree-sha> <head-sha>` scans every reachable commit for initial-push receipt changes.
...
- The push-range resolver and the commit-identity checker must match their supported digests. For a normal push, `identity-gate` resolves the merge base with code extracted from the checked-out protected base. A zero `before` SHA or parentless pushed commit uses the empty tree for range checks, validates every reachable commit, checks out the pushed revision as the explicitly trusted initial base, and records that decision in the job summary.
...
- `control-gate` checks out only the protected base for normal pushes, fetches the candidate as Git data, and runs the control verifier and its complete local import closure extracted from that base tree. On an initial push it checks out the pushed revision as the explicitly trusted base and records the decision in the job summary. It validates candidate `v4-gates.yml`, `fork-proof.yml`, `fork-pin-canary.yml`, `identity-gate.yml`, `control-gate.yml`, `scripts/ci/push-range.mjs`, the complete fork-pin verifier closure, the complete control-verifier closure, the archive fork test, and `dependency-pins.json`. A pin bump requires an exact owner-approved `controlGatePinBump` record bound to the base checker blob, tree hashes, pin bytes, and changed digest set.
...
- The `pull-request` and `push` jobs in each base-defined workflow use the explicit display names `control-gate` and `identity-gate`; configure branch rules with those exact check names.
...
- Non-initial push ranges require the previous SHA to be an ancestor of the new SHA. Initial pushes use the empty tree as the range base and scan every commit reachable from the pushed SHA. Every new receipt is a regular `100644` Git blob; every prior receipt remains byte-identical across every commit and merge parent.
...
3. Pull requests run `identity-gate`, `control-gate`, and `gates` without endpoint access. Normal main pushes extract range controls from the protected base; initial main pushes use the empty tree range and record the explicitly trusted pushed base.
...
node scripts/ci/push-range.mjs resolve <before-sha> <head-sha> <before|merge-base> <output-file> [summary-file]
node scripts/ci/push-range.mjs append-only <empty-tree-sha> <head-sha>
...
- A non-fast-forward non-initial main update is rejected; restore a forward-only history through a reviewed branch.
- For an initial-push range failure, verify the event's `before` SHA and the pushed commit parents, then retain the empty-tree range. Do not replace the range checks with a skip.
- OPEN FACT — this repository's existing root predates the resolver. Installing it needs an owner-authorized bootstrap transition because the protected-base verifier cannot validate a pin record that must bind the candidate commit containing that record. Once installed, later pushes use the base-defined resolver.
