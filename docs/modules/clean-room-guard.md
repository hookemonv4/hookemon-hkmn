...
- Retired stablecoin markers remain rejected except for the exact case-insensitive "Solana <stablecoin>" prose adjacency, the exact typed Solana mainnet SPL mint form, the audited catalog fields and formatter in the operator panel, and two complete legacy accounting field identifiers. Any other spelling, chain, suffix, file, or identifier boundary fails closed.
