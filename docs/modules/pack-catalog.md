...
List responses contain `provider`, `sourceUrl`, `fetchedAt`, `valueType`, `availabilityNotice` and `packs`. Each pack includes `code`, `name`, `category`, `price`, `currency`, `contains`, `availability`, `sourceUrl`, nullable `image` and `tiers` with `rarity`, `minimum`, `maximum`. Inventory adds `code`, `rarity`, `availability`, `cards`, `page`, `pageSize` and `hasMore`. Cards expose name, rarity, insured value, grade, certificate, year, category, variant, population, vault and provider record links. Missing optional details are null.
