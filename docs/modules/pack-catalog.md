...
List responses contain `provider`, `sourceUrl`, `fetchedAt`, `valueType`, `availabilityNotice` and `packs`. Each pack includes `code`, `name`, `category`, `price`, `currency`, `contains`, `availability`, `sourceUrl`, nullable `image`, nullable `instantBuybackPercent` and `tiers` with `rarity`, `minimum`, `maximum`. Inventory adds `code`, `rarity`, `availability`, `instantBuybackPercent`, `buybackNotice`, `cards`, `page`, `pageSize` and `hasMore`. Cards expose name, rarity, insured value, grade, certificate, year, category, variant, population, vault and provider record links. Missing optional details are null. Each card includes nullable `estimatedBuybackUsd`, rounded to cents from its insured value and the inventory response’s pack percentage. The percentage comes from `instantBuyback.percentageOfValue`; missing, non-numeric, zero, negative or over-100 rates produce null estimates. The browser labels estimates separately from insured values and states that actual offers may vary.

Provider origin, category, currency, page size and allowed image hosts live in `config/pack-provider.json`. All provider requests use fixed HTTPS URLs, GET, redirect rejection and a timeout. Only public, unarchived packs in the exact `Pokemon` category are listed. Machine status and per-pack status are both consulted; absent status is unknown. Image origins are allowlisted. Pack covers prefer `imageNobgUrl`, then `image`, then `thumbnailUrl`; missing or failed artwork falls back to the Hookemon symbol. Provider strings enter the browser through textContent. No wallet, purchase, signature or personal account endpoints are used.

Rarity filters and card image backgrounds share the same tier tint: Common grey-green, Uncommon green, Rare blue and Epic purple. Selection keeps that tint and adds an inset accent ring and heavier label. Filters retain a minimum 44px touch target. The first accordion expansion loads only the Epic tier's first page; the other tiers and subsequent pages load on demand. Tier selection cancels previous requests. Provider failures clear displayed stock rather than silently substituting cached or example cards. Refresh reloads the catalogue. Responses use `no-store`. `fetchedAt` is our retrieval time, not a provider valuation date. Inventory can change between pages or before purchase. Cards returned on overlapping pages are deduplicated by NFT ID. Category tiers are distinct from printed card rarity; the UI does not infer per-card odds from inventory counts or weights.
