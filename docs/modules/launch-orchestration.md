...
Launch Orchestration prepares the native ETH and HKMN market and its permanent liquidity position. The current custom adapter preparation separates the three-target Programmable graph from the later seed call, which may carry zero native value. Provider-supported atomic initialization remains unverified. Local derivation, draft validation and package verification neither approve a launch nor sign or broadcast it.
...
`HookemonHook.seedCanonicalLiquidity(SeedParams)` accepts native ETH with `msg.value == amount0Max`. Only HKMN uses temporary approvals. Zero-native inventory mints maximal liquidity at its selected upper tick and locks mechanical HKMN rounding dust in permanent custody. Positive-native legacy seeds consume the full stock in their full-range position and refund unused native value to the payer, who may differ from the caller. Custody permanently binds the verified position.
...
`PhaseThreeReleasePlan.validateDraft` checks source, roles, allocation and fees. Zero-native inventory requires explicit aligned bounds, initialization at the upper price, maximal token-only liquidity and valid signed/per-tick limits; mechanical rounding dust stays in permanent custody. Funded seed tuples must pay the full-stock debt within their maximum. Validation does not select or approve a production price. Address derivation and package verification bind the deterministic candidate.
...
The native derivation tools distinguish the explicit string `seedMaximumWei: "0"` from absent inputs. Zero selects token-only inventory using supplied, aligned tick bounds and tick spacing. `deriveTokenInventoryCandidate` computes price at the upper tick, maximal floor(stock * Q96 / span) liquidity, zero native debt and the rounded token debt plus `lockedHkmnDust`. It checks signed-liquidity and pinned per-tick bounds. That mechanical remainder belongs to permanent custody, never to a chosen reserve or treasury allocation. Positive maxima retain the funded full-range candidate unchanged.

Materialized address inputs bind the selected range through `seedIntent`; release inputs retain the historical `pool.fullRange` field name for those explicit bounds. Both bounds may remain null only in an unresolved zero-native draft with a null price candidate. Materialization rejects missing ranges, price mismatch, mismatched native value and changed seed-policy digests. No range, payer, transaction deadline, nonce or wei claim ceiling is supplied by derivation. The first buyer needs explicit pre-settlement through the selected router; package generation does not establish provider compatibility.

The release-closure fee-ordering check accepts a zero-native draft without a price candidate only when both range bounds, selected price, PoolKey and PoolId remain null, selection is OPEN_FACT and nativeFunding is explicitly zero. A partially selected range, positive funding without a candidate or a materialized selection with no candidate is rejected. File, source-tree and vendored-builder digest verification remains unchanged.
