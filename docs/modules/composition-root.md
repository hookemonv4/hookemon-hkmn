...
The saved `packPlan` drives new live cycle admission. The planner captures the selected revision, reads one catalog, validates every selected machine, obtains each unit quote sequentially and then requests a real exact-output quote for the combined settlement amount. It never estimates aggregate funding by multiplying a unit quote. The current persisted selection is not consulted during an existing cycle's recovery. Claim, outbound, return and accounting explicitly recognize normalized native admission v4; historical USDG admissions do not enter the native path.

`readPackPlan` is bound to the automated service only together with the production admission planner (live production mode with native money configuration and both Operations accounts). A live composition without that planner, such as the Collector-only rehearsal, opens its cycle from the configured `pack.code` and reaches its own durable-admission refusal at the money boundary; it does not report `WAITING_FOR_ADMISSION` on the empty saved selection. A production composition reports `WAITING_FOR_ADMISSION` when its saved plan is empty or its planner declines to admit.
