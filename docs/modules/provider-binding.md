...
`prepareNativeCommitmentInputs` checks the exact revision-72 bytes and pinned compiler distribution, collects supplied build bytes and derives the authenticated observation's runtime commitment. It does not return a constructor binding or provider coordinates. Complete inventory and compiler-output reproduction must precede independent token/custody derivation with the checksum-verified official provider modules. Existing repository bundle-hash helpers are not substitutes for those official content-byte preimages.
