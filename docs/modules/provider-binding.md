...
- `RobinhoodBindings` and `ImmutableLaunchBinding` pin native ETH, PoolManager, PositionManager, Permit2, the graph factory, the launch-stamp router, the Operations identity, the `0x20CC` hook permission mask, zero static LP fee, and the 20/30/250 basis-point fee split. Binding validation rejects a mismatched value or digest.
...
- The selected assisted-launch allocation is inclusive 20/30/250 bps; `FEE-01` records 10 basis points historically. Programmable receives `0xD88539d3c4C460136a733A3Fd60cf6BF269079da`. Current native integration must verify the selected custom model without an extra surcharge.
...
`readNativeRequirementsBytes(root)` reads only the exact frozen revision-76 requirements and refuses changed bytes or a linked requirements file or specs directory. Revision 76 applies the latest Treasury correction; revision 75 recorded the preceding platform fee revision; historical revision 74 applied the owner-approved recipient-restoration delta after native revision 73; the approval is retained in `docs/proposals/reward-recipient-restoration/owner-approval.md`. This byte check grants no runtime or launch authority.
...
The provisional native build record binds revision 76 and its inclusive 20/30/250 fee split to exact current input hashes. Historical freeze and approval hashes retain their original values. This build record cannot grant production readiness, provider admission or a live interface freeze.
