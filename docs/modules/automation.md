...
## Reserved manual cycle

The optional `manualCycleId` reserves one cycle identity for `runOnce` and constrains recovery to that identity. Manual calls do not dispatch unrelated supplementary sales or payout retries. Admission checks the request's frozen pack plan, recipient limit and configuration revision under the existing execution lease. Existing policy, custody and mutation guards remain required. These parameters do not provide direct-wallet funding or external-token eligibility authority.
