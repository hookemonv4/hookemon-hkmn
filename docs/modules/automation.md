...
A composed live service reads `readPackPlan()` once when opening a cycle. Empty plans admit nothing. The same canonical plan supplies quote admission and the atomic cycle snapshot; subsequent ticks and restarts derive pack identity from that cycle's admission. Each later cycle reads the saved plan again, so selections repeat until edited and edits affect only future cycles.
...
The composed live service reads one authoritative operator configuration for the next cycle, freezing its pack plan and reward recipient limit before creation. `readCycleConfiguration` cannot coexist with the legacy `readPackPlan` callback. Resumed stages receive only the stored cycle reward policy; changed operator settings apply to subsequent cycles. A missing configured authority starts no cycle.
