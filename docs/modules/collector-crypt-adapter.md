...
Purchase reconciliation holds unresolved provider status, signature lookup or settlement evidence after the deadline. Open reconciliation likewise keeps querying every pack, preserves pending failures, and records an unverifiable card only after its recorded response or sent-unknown deadline; a legacy timestamp-less `SENT_UNKNOWN` or `RESPONSE_RECORDED` attempt is durably anchored on first observation, with `respondedAtMs` taking precedence over `sentAtMs`, then `deadlineAnchorMs`. That anchor survives restart and starts a fresh deadline window; a provider or RPC outage never triggers a retry mutation or silently skips later packs. Missing provider records alone cannot establish that no purchase debit occurred.
