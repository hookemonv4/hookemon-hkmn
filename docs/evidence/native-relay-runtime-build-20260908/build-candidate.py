...
cmd=['docker','--config',str(root/'.session/relay-runtime/docker-public'),*(['--host',os.environ['DOCKER_HOST']] if os.environ.get('DOCKER_HOST') else []),'run','--rm','--platform','linux/amd64','-v',f'{src}:/build','-w','/build/programs/relay-depository',image,'bash','-c','rustc --version && solana --version && cargo build-sbf --version && cargo build-sbf -- --config \'registries.crates-io.protocol="sparse"\' --locked']
