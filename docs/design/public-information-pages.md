...
The cycle page reuses the homepage's strict public-dashboard validators, completion-aware payout helper, history completeness rule and exact integer amount formatting. It fetches both observations together with a bounded timeout. A refresh failure clears the previous financial presentation. Transaction URLs are derived only after the payload's profile, network and transaction identity are validated. Collector-side observations retain their reported chain and token identity; no conversion or network relabelling occurs. The available source exposes a latest record, not a complete per-cycle archive.
