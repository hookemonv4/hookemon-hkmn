...
function fixture(programmableFeeBps = '20', treasuryFeeBps = '30') {
  ...
  const binding = { schema: 'hookemon.native-issuance-prebinding.v1', chainId: '4663', genesisHash: h,
    ...
    economics: { name: 'Hookemon', symbol: 'HKMN', decimals: '18', totalSupplyAtomic: launchInputs.pool.hkmnAtomic,
      ...
      programmableFeeBps, treasuryFeeBps, hookPermissionMask: '8396',
...
test('native release economics rejects the superseded 40-bps Treasury allocation', () => {
  const f = fixture('20', '40');
  try {
    assert.throws(() => verifyNativeReleaseIdentities(f), /economics treasuryFeeBps/);
  } finally { rmSync(f.inputDirectory, { recursive: true, force: true }); }
});
