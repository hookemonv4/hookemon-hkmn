...
test('the machine-readable delivery boundary keeps Phase 3 open under native revision 73 and architecture revision 11', () => {
  const boundary = readJson('product/delivery-boundary.json');
  ...
  assert.equal(requirements.revision, 73);
  assert.equal(readJson('feasibility/phase3-offchain-interface-amendment.json').approvedRequirementsRevision, 70,
    'the retained amendment is historical and does not approve native revision 73');
  assert.equal(interfaces.productPhase, 3);
  assert.equal(interfaces.requirementsRevision, 73);
...
test('native Phase 3 interfaces retain the provisional module registry without freeze authority', () => {
  ...
  assert.equal(provisional.requirementsRevision, 73);
