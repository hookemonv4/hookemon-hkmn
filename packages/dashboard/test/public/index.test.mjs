...
async function packPage({ catalog, selected = ['old-pack'], response } = {}) {
  ...
  const page = browserHarness({
    ...
    async get(path) {
      if (path.endsWith('/bootstrap')) return { state: { version: revision, allowedPackIds: saved } };
    ...
    response: response ?? (() => {
      saved = requests.at(-1).command.configuration.allowedPackIds;
      revision += 1;
...
test('pack selection lists the catalog, preserves missing selections and saves sorted unique IDs', async () => {
  ...
  assert.deepEqual(requests[0], {
    ...
    command: { type: 'update-configuration', configuration: { allowedPackIds: ['aa-pack', 'old-pack', 'zz-pack'] } },
  });
  assert.equal(page.element('packMessage').textContent, 'Pack selection saved.');
  assert.equal(page.element('savePacksBtn').disabled, true);
  page.click('clearPacksBtn');
  page.click('savePacksBtn');
  await settlePage();
  assert.deepEqual(requests[1].command.configuration.allowedPackIds, []);
