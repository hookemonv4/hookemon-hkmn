...
contract OwnerCapitalFeasibilityTest is Test {
    ...
    function test_onePackFundedByActualFeesWith150UsdgInitialCapital() public {
        ...
        hook = new AccountedCanonicalMarketHookDeployer().deploy(manager,Currency.wrap(address(usdg)),Currency.wrap(address(hkmn)),address(0xC001),address(0xC002));
        ...
        uint128 liquidity = LiquidityAmounts.getLiquidityForAmounts(price,TickMath.getSqrtPriceAtTick(-887220),TickMath.getSqrtPriceAtTick(887220),100_000_000,1_000_000_000 ether);
        uint256 seedBefore = usdg.balanceOf(address(this));
        liquidityRouter.modifyLiquidity(key,ModifyLiquidityParams(-887220,887220,int256(uint256(liquidity)),bytes32(0)),"");
        ...
        while(hook.processLiability()<25_298_644 && swaps<100) {
            ...
            router.swap(key,SwapParams(true,-int256(available),TickMath.MIN_SQRT_PRICE+1),address(this),address(this),"");
            ...
            start=gasleft();
            router.swap(key,SwapParams(false,-int256(purchased),TickMath.MAX_SQRT_PRICE-1),address(this),address(this),"");
            gasUsed += start-gasleft()+21_000;
            gross += hook.lastExecutedUsdg();
            swaps++;
            ...
            assertEq(usdg.balanceOf(address(this))+usdg.balanceOf(address(manager))+usdg.balanceOf(address(hook)),150_000_000);
