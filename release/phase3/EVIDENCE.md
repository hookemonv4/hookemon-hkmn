...
`native-interface-build-binding.json` under `feasibility/` binds the revision-73 build inputs provisionally. Its historical freeze and offchain amendment provide no current native feasibility credit. `verify-launch-package.mjs --allow-unverified` checks draft consistency while returning `readyForPreflight:false`; it is not a release pass. `review-target.json` inventories package files and exact digests.
...
The owner retains the 10 bps Programmable share after direct provider discussion (r-00655) and limits the first run to at most ten transactions. Programmable acceptance of this fee is owner-confirmed in r-00656 and is not an open approval question. Any API response is recorded separately; source/runtime and transaction validation remain required.
